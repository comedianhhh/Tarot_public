...
namespace MoreMountains.CorgiEngine
{
    ...
    public class Health : MonoBehaviour
    {
        ...
        protected Renderer[] _flickerRenderers = null;
        /// <summary>
            ...
            if (_character != null)
            {
                if (_character.CharacterModel != null)
                {
                    if (_character.CharacterModel.GetComponentInChildren<Renderer>() != null)
                    {
                        ...
                        _flickerRenderers = _character.CharacterModel.GetComponentsInChildren<Renderer>();
                    }
                }
            }
            ...
            float invincibilityDuration, Vector3 damageDirection)
        {
            ...
            if (FlickerSpriteOnHit)
            {
                ...
                if (_renderer != null)
                {
                    if (_flickerRenderers != null)
                    {
                        foreach (var rend in _flickerRenderers) StartCoroutine(MMImage.Flicker(rend, _initialColor, FlickerColor, 0.05f, flickerDuration));
                    }
                    else
                    {
